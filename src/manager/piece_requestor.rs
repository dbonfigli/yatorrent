use crate::{
    ...
    persistence::{file_manager::FileManagerStatus, piece::Piece},
...
impl PieceRequestor {
    ...
    pub fn generate_requests_to_send(
        &mut self,
        peers: &HashMap<String, Peer>,
        file_manager_status: &FileManagerStatus,
        ...
        for (piece_idx, piece) in file_manager_status.incomplete_pieces().iter() {
        ...
        for piece_idx in file_manager_status.missing_pieces() {
            ...
            match self.assign_piece_reqs(
                *piece_idx,
                peers,
                &Piece::new(file_manager_status.piece_length(*piece_idx)),
            ) {
