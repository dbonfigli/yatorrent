...
impl PieceRequestor {
    ...
    pub fn generate_requests_for_allow_fast(
        &mut self,
        peer_addr: &String,
        peer: &Peer,
        piece_idx: usize,
        ...
        if !(peer.client_version.is_some()
            && peer
                .client_version
                .as_deref()
                .unwrap()
        ...
        // piece idx is not valid or already downloaded
        if piece_idx >= file_manager.current_piece_completion_status().len()
            || file_manager.current_piece_completion_status()[piece_idx]
        {
            return Vec::new();
        }

        // piece is already assigned to someone else
        if let Some(peer_addr_with_this_piece_assigned) =
            self.outstanding_piece_assignments.get(&piece_idx)
        {
            if peer_addr_with_this_piece_assigned != peer_addr {
                return Vec::new();
            }
        }

        // if peer does not really have this piece
        // todo: are we sure we need to check this?
        if !peer.have_piece(piece_idx) {
            return Vec::new();
        }
        ...
        let piece = if let Some(requested_pieces_for_peer) = self.requested_pieces.get(peer_addr) {
            if let Some(piece) = requested_pieces_for_peer.get(&piece_idx) {
                piece.clone()
            } else if let Some(piece) = file_manager.incomplete_pieces().get(&piece_idx) {
                piece.clone()
            } else {
                Piece::new(file_manager.piece_length(piece_idx))
            }
        ...
        let requests_to_send =
            self.generate_requests_to_send_for_piece(peer_addr, piece_idx, piece, request_count);

        return requests_to_send;

        // todo we are now risking stalled requests never removed for this assigments here
        // need to clean them up after a while
        // also need to assign missing blocks for this pieces later
    }

    pub fn block_request_rejected(&mut self, peer_addr: &PeerAddr, block_request: &BlockRequest) {
        if let Some(reqs) = self.outstanding_piece_block_requests.get_mut(peer_addr) {
            if let None = reqs.remove(block_request) {
                log::debug!(
                    "we received a reject request {:?} from {peer_addr} but request was not tracked",
                    block_request
                );
            }
        } else {
            log::debug!(
                "we received a reject request {:?} from {peer_addr} but request was expired (no outstanding piece requests from this peer at all)",
                block_request
            );
        }

        // todo: should we remove piece assigment here? in theory this will be handled by a choke handling
        // but if does not, now we have a problem: we need to remove outstanding_piece_assignments, it not other blocks were requested for this piece,
        // but also change requested_pieces, and this is really hard
    }
}
